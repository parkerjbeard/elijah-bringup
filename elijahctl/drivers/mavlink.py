...
class MAVLinkDriver:
    ...
    def set_parameter(
        self, param_name: str, value: float, param_type: int = mavutil.mavlink.MAV_PARAM_TYPE_INT32
    ) -> bool:
        ...
        try:
            ...
            self.connection.mav.param_set_send(
                self.target_system, self.target_component, param_id_bytes, float(value), param_type
            )
            ...
            while time.time() - start_time < 5:
                ...
                if msg and msg.param_id.strip("\x00") == param_name:
                    if abs(msg.param_value - value) < 0.001:
                        ...
                        warning(
                            f"Parameter value mismatch: expected {value}, got {msg.param_value}"
                        )
    ...
    def get_parameter(self, param_name: str) -> Optional[float]:
        ...
        try:
            ...
            self.connection.mav.param_request_read_send(
                self.target_system, self.target_component, param_id_bytes, -1
            )
    ...
    def set_sysid(self, sysid: int) -> bool:
        ...
        if not self.set_parameter("MAV_SYS_ID", float(sysid), mavutil.mavlink.MAV_PARAM_TYPE_INT32):
