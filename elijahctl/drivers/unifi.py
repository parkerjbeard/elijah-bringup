...
class UniFiDriver:
    ...
    def _detect_api_version(self) -> Optional[str]:
        try:
            ...
            response = self.session.get(f"{self.config.controller_url}/api/s/{self.site_name}/stat/sysinfo", timeout=5)
            if response.status_code == 200:
                return "v6"
    ...
    def _is_unifi_os(self) -> bool:
        ...
        try:
            ...
            self._unifi_os = r.status_code == 200 and isinstance(r.json(), dict) and "meta" in r.json()
        except Exception:
            self._unifi_os = False
        return self._unifi_os

    def _net_base(self) -> str:
        return f"{self.config.controller_url}/proxy/network" if self._is_unifi_os() else self.config.controller_url
    ...
    def disable_auto_optimize(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(settings_url, json=settings_data, timeout=Config.HTTP_TIMEOUT)
            ...
            try:
                ...
                if r.status_code == 200:
                    for wlan in r.json().get("data", []):
                        bands = wlan.get("wlan_bands") or ([wlan.get("wlan_band")] if wlan.get("wlan_band") else [])
