...
class JetsonDriver:
    ...
    def _run_ansible_playbook(self) -> Tuple[bool, str]:
        ...
        cmd = [
            ...
            "60",
            "-e",
            f"ansible_host={self.config.ansible_host}",
            "-e",
            f"ansible_user={self.config.ansible_user}",
            "-e",
            f"device_name={self.config.device_name}",
            "-e",
            f"sysid={self.config.sysid}",
            "-e",
            f"tailscale_auth_key={self.config.tailscale_auth_key}",
            "-e",
            f"microhard_password={self.config.microhard_password}",
        ]
        ...
        if shutil.which("sshpass"):
            cmd = (
                ...
                + cmd
                + ["--extra-vars", f"ansible_become_pass={self.become_pass}"]
            )
        else:
        ...
        try:
            ...
            env.setdefault(
                "ANSIBLE_SSH_ARGS",
                "-o ControlMaster=auto -o ControlPersist=60s -o ServerAliveInterval=15 -o ServerAliveCountMax=2",
            )
    ...
    def _upload_firmware(self, firmware_path: Optional[Path] = None) -> bool:
        ...
        try:
            subprocess.run(
                ssh_base
                + [f"{self.config.ansible_user}@{self.config.ansible_host}", "mkdir -p", dest_dir],
                check=False,
            )
        except Exception:
            pass
    ...
    def provision(self, firmware_path: Optional[Path] = None) -> bool:
        try:
            ...
            if not ts_ok:
                warning(
                    "Tailscale verification failed - this is expected if radio is not connected"
                )
