...
class TestHealthCheck:
    ...
    def test_check_tailscale_online(self, mock_ssh_client, health_check):
        ...
        mock_stdout.read.return_value = json.dumps(
            {"Self": {"Online": True, "DNSName": "el-012.tail-scale.ts.net"}}
        ).encode()
    ...
    def test_run_all_checks(
        ...
        mock_result = HealthCheckResult(component="Test", status=True, message="Success")
    ...
    def test_check_pth_sensors_success(self, mock_ssh_client, health_check):
        ...
        mock_stdout = MagicMock()
        mock_stdout.read.return_value = json.dumps(
            {"pressure": 1013.25, "temperature": 22.5, "humidity": 45.0}
        ).encode()
