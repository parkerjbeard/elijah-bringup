...
class TestIntegration:
    ...
    def test_discover_command(self, mock_discover, runner):
        mock_discover.return_value = {"ssh": True, "http": True, "telnet": False}
    ...
    def test_provision_ground_radio(self, mock_provision, runner):
        ...
        result = runner.invoke(
            cli, ["provision", "--role", "ground", "--drone-id", "001", "--aes-key", "test_key_128"]
        )
    ...
    def test_provision_air_without_sysid(self, runner):
        result = runner.invoke(
            cli, ["provision", "--role", "air", "--drone-id", "012", "--aes-key", "test_key_128"]
        )
    ...
    def test_reset_radio(self, mock_reset, runner):
        ...
        result = runner.invoke(cli, ["reset-radio", "--ip", "192.168.168.1"], input="y\n")
    ...
    def test_health_check(self, mock_save, mock_run_checks, runner):
        mock_run_checks.return_value = [Mock(status=True), Mock(status=True), Mock(status=False)]
        mock_save.return_value = "/tmp/results.json"

        result = runner.invoke(cli, ["health", "--jetson", "el-012", "--radio-ip", "10.0.0.100"])
    ...
    def test_checklist_update(self, runner, temp_dir):
        ...
        result = runner.invoke(
            cli,
            ["checklist", "--update", str(checklist_file), "--drone-id", "012", "--phase", "hitl"],
        )
    ...
    def test_checklist_export(self, runner, temp_dir):
        ...
        result = runner.invoke(
            cli, ["checklist", "--drone-id", "012", "--export", str(export_file)]
        )
    ...
    def test_set_sysid(self, mock_set_sysid, runner):
        ...
        result = runner.invoke(cli, ["set-sysid", "--host", "el-012", "--sysid", "12"])
