...
class HealthCheck:
    ...
    def _add_result(
        self, component: str, status: bool, message: str, data: Optional[Dict] = None
    ) -> HealthCheckResult:
        result = HealthCheckResult(component=component, status=status, message=message, data=data)
    ...
    def check_connectivity(self) -> Dict[str, HealthCheckResult]:
        ...
        if jetson_ok:
            ...
            results["jetson"] = self._add_result("Jetson Connectivity", False, "Not reachable")
        ...
        if self.radio_ip:
            ...
            if radio_ok:
                ...
                results["radio"] = self._add_result("Radio Connectivity", False, "Not reachable")
    ...
    def check_tailscale(self) -> HealthCheckResult:
        ...
        try:
            ...
            if output:
                ...
                if self_info.get("Online"):
                    ...
                    return self._add_result("Tailscale", False, "Not online")
        ...
        return self._add_result("Tailscale", False, "Check failed")
    ...
    def check_radio_stats(self, timeout: int = 5) -> HealthCheckResult:
        ...
        return self._add_result("Radio Stats", False, "No data received")
        ...
        try:
            ...
            if mavlink.connect(timeout=10):
                ...
                if heartbeat_ok:
                    ...
                    return self._add_result(
                        "MAVLink", False, f"Heartbeat rate too low ({count} in 5s)"
                    )

        except Exception as e:
            logger.error(f"MAVLink check failed: {e}")

        return self._add_result("MAVLink", False, "Connection failed")
    ...
    def check_video_stream(self) -> HealthCheckResult:
        ...
        try:
            ...
            if spec.startswith("udp:"):
                ...
                try:
                    ...
                    return self._add_result(
                        "Video Stream", False, f"UDP {port}: no packets received"
                    )
                finally:
                    try:
                        sock.close()
            ...
            # RTSP mode: rtsp://host[:port]/...
                ...
                if rc == 0:
                    ...
                    return self._add_result(
                        "Video Stream", False, f"RTSP {host}:{port}: not reachable"
                    )
        ...
        return self._add_result("Video Stream", False, f"No stream ({spec or 'unspecified'})")
    ...
    def check_pth_sensors(self) -> HealthCheckResult:
        ...
        try:
            ...
            stdin, stdout, stderr = client.exec_command(
                f"cat {pth_path} 2>/dev/null || echo '{{}}'"
            )
            output = stdout.read().decode("utf-8")
            client.close()
            ...
            if output and output != "{}":
                try:
                    ...
                    if (
                        "pressure" in pth_data
                        and "temperature" in pth_data
                        and "humidity" in pth_data
                    ):
                        return self._add_result(
                            "PTH Sensors",
                            True,
        ...
        return self._add_result("PTH Sensors", False, "Sensor data not available")
    ...
    def check_versions(self) -> Dict[str, HealthCheckResult]:
        ...
        try:
            ...
            stdin, stdout, stderr = client.exec_command(
                "cd /opt/seraph && git rev-parse HEAD 2>/dev/null || echo 'unknown'"
            )
            seraph_version = stdout.read().decode("utf-8").strip()

            stdin, stdout, stderr = client.exec_command(
                "cd /opt/elijah && git rev-parse HEAD 2>/dev/null || echo 'unknown'"
            )
            elijah_version = stdout.read().decode("utf-8").strip()

            stdin, stdout, stderr = client.exec_command(
                "cat /opt/firmware/version.txt 2>/dev/null || echo 'unknown'"
            )
    ...
    def run_all_checks(self) -> List[HealthCheckResult]:
        ...
        # Run in parallel to reduce wall time; also aggregate returned results
        returns: list[Any] = []
        with ThreadPoolExecutor(max_workers=4) as ex:
            returns = list(ex.map(lambda f: f(), funcs))

        # Collect any returned HealthCheckResult objects from patched/mocked functions
        try:
            ...
            for item in returns:
                if isinstance(item, HealthCheckResult):
                    ...
                    for v in item.values():
                        if isinstance(v, HealthCheckResult):
