...
from .config import (
    Config, HitlChecklist, InsituChecklist, 
    HealthCheckResult
)
...
class ChecklistManager:
    ...
    def update_from_health_check(self, results: List[HealthCheckResult]):
        ...
        all_health_checks_passed = all(r.status for r in results if r.component in [
            "MAVLink", "Radio Stats", "PTH Sensors", "Video Stream"
        ])
    ...
    def generate_labels(self, drone_id: str) -> Dict[str, str]:
        ...
        if self.current_hitl:
            labels["serial"] = self.current_hitl.serial_qr[:10] if self.current_hitl.serial_qr else "N/A"
